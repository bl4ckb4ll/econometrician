...
At geometric-uncertainty build time: **40 package tests passed**. The complete
repository collection reports **95 tests passed**.
...
The sibling legacy `../dakota-caster-recheck/dakota_caster_recheck.py` must
preserve scientific fields and nonnumeric values while reproducing every float
within `rtol=1e-6`, `atol=1e-8`. Solver outer-iteration count is diagnostic
metadata and is not an oracle. The tolerance admits cross-runner
linear-algebra variation without admitting a change at the four-decimal
precision used by the historical conclusions. That legacy receipt is
deliberately separate from the new benchmark tests because it validates old
implementation reproducibility, not source correctness.

The pinned Idriç structural program is also checked separately. It passes the
source-conservation, nested-neighborhood, sphere-tangent, axial-kernel,
odd/even-reversal, and robust-intake checks. Its companion Float32 probe is an
expected compile-time boundary, not a passing test: the pinned compiler reports
`Undefined name Float32`. Exact commands and evidence are retained in
[`attempt.md`](../../../examples/autogenerated/dakota-geometric-uncertainty/attempt.md).

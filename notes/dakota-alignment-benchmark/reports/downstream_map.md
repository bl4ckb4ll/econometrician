...
The old robust-fit coefficients are suitable only as legacy regression fixtures for checking that old code still reproduces itself. They are not reusable Dakota facts.

## Recovered repository lineage

Dakota numerical snapshots were merged into `the-twin-pines/ASE` PR #5 (`2ec8f4a9…`) and `bl4ckb4ll/econometrician` PR #3 (`b992a1b3…`). Their own issue/PR text keeps actual vehicle caster unknown and treats the uploaded fit as numerical/research infrastructure. A separate `isomorphismes/yt-shorts` Gromov/curvature branch exists, but no evidence recovered here ties its object definitions to a Dakota-specific curvature Jacobian.

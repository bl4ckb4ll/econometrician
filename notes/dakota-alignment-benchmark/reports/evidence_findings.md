...
3. **The old script is internally reproducible.** Rerunning it preserves
   scientific fields/text and reproduces floats within `rtol=1e-6`,
   `atol=1e-8`; solver outer-iteration count is diagnostic metadata.
   Cross-runner linear-algebra variation below the four-decimal historical
   reporting precision is not treated as a substantive change. That proves
   software repeatability only; its data mapping and illustrative error model
   remain disputed.

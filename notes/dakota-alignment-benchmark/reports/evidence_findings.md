...
3. **The old script is internally reproducible.** Rerunning it preserves exact JSON structure/text and reproduces floats within `rtol=1e-9`, `atol=1e-10`; last-bit linear-algebra variation across runners is not treated as a substantive change. That proves software repeatability only; its data mapping and illustrative error model remain disputed.

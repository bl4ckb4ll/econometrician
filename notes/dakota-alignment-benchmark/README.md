...
**Audit version:** 2026-09-21-geometric-uncertainty-1
...
### 7. The no-yaw procedure now has an explicit geometric uncertainty map

Direct road-wheel yaw is not required. Steering command enters a declared
family `G(command, side, path, state)` whose nominal member uses the documented
17.4:1 ratio. Wheel normals live on `S^2`, steering rotations in `SO(3)`, and
the gauge is a projection against the reference/gravity frame. Every
identified error source is registered before fitting and must receive a
propagation disposition. Unknown input-map error remains symbolic or set-valued;
it is not discarded or manufactured into a Gaussian sigma.
...
19. **Geometric uncertainty pushforward** —
    `reports/geometric_uncertainty_pushforward.md`,
    `data/caster_error_source_census.tsv`, and the exact structural Idriç
    attempt in `../../examples/autogenerated/dakota-geometric-uncertainty/`.
...
- 40 benchmark tests pass;
- the recovered legacy caster script preserves scientific fields and text,
  reproduces floats within `rtol=1e-6`, `atol=1e-8` across runners, and treats
  solver iteration count as diagnostic metadata.
...
- source-conserving geometric uncertainty and no-yaw steering-map experiments.

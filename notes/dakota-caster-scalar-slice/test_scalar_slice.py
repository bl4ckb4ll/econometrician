...
class ScalarSliceTests(unittest.TestCase):
    ...
    def test_raw_observation_identity_and_order(self) -> None:
        ...
        self.assertEqual(
            [
                (r["evidence_id"], r["steering_position"], r["raw_value_deg"])
                for r in rows
            ],
            [
                ("E-026", "0.5 turn right", 0.5),
                ("E-028", "0.5 turn left", 2.5),
            ],
        )
    ...
    def test_side_generation_and_sign_metadata(self) -> None:
        ...
        self.assertFalse(
            self.evidence["metadata"][
                "current_viewpoint_is_retroactive_historical_claim"
            ]
        )
    ...
    def test_cam_state_gap_is_preserved_not_invented(self) -> None:
        self.assertIn(
            "not recovered",
            self.evidence["metadata"]["eccentric_cam_position_for_selected_pair"],
        )
    ...
    def test_symmetric_multiplier(self) -> None:
        theta = math.radians(self.x[0])
        multiplier = 1.0 / (2.0 * math.sin(theta))
        self.assertAlmostEqual(multiplier, 2.784399371553343, places=12)

    def test_signed_and_magnitude_arithmetic(self) -> None:
        signed = s.signed_caster_coefficient(*self.x)
        magnitude = s.caster_magnitude(*self.x)
        self.assertAlmostEqual(signed, -5.568798743106686, places=12)
        self.assertAlmostEqual(magnitude, 5.568798743106686, places=12)

    def test_sign_reversal_if_camber_endpoints_swapped(self) -> None:
        swapped = [self.x[0], self.x[1], self.x[3], self.x[2]]
        self.assertAlmostEqual(
            s.signed_caster_coefficient(*swapped),
            -s.signed_caster_coefficient(*self.x),
            places=12,
        )
        self.assertAlmostEqual(
            s.caster_magnitude(*swapped),
            s.caster_magnitude(*self.x),
            places=12,
        )
    ...
    def test_expected_jacobian(self) -> None:
        ...
        for actual, target in zip(j, expected):
    ...
    def test_historical_symmetric_pair_agreement(self) -> None:
        comparison = self.result["historical_comparison"]
        self.assertTrue(comparison["recomputed_inside_reported_range"])
        self.assertEqual(comparison["result"], "agreement")

    def test_printed_multiplier_rounding_discrepancy_is_small_and_explicit(self) -> None:
        comparison = self.result["historical_comparison"]
        self.assertAlmostEqual(
            comparison["result_using_printed_multiplier_deg"], 5.568, places=12
        )
        self.assertAlmostEqual(
            comparison["exact_minus_printed_multiplier_result_deg"],
            0.0007987431066858186,
            places=12,
        )

    def test_legacy_illustrative_sigmas_are_not_used_as_inputs(self) -> None:
        excluded = self.result["uncertainty_boundary"][
        ...
        text = json.dumps(self.result["calculation"], sort_keys=True)
        self.assertNotIn("sigma", text)
    ...
    def test_cli_output_is_byte_deterministic(self) -> None:
        ...
        a = subprocess.check_output(cmd, cwd=HERE)
        b = subprocess.check_output(cmd, cwd=HERE)

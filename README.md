...
The Haskell endpoint-pair frontend is now the measurement-facing IEEE-754
binary32 path; R, Ithon, Agda, Idriç, and NumPy remain binary64
regression/reference implementations. The source-conserving geometric
procedure, remaining Float16/Float32 boundary, nested uncertainty sets,
`S^2`/`SO(3)` tangent maps, and robust intake boundary are documented in
[`geometric_uncertainty_pushforward.md`](notes/dakota-alignment-benchmark/reports/geometric_uncertainty_pushforward.md),
with an exact structural Idriç slice under

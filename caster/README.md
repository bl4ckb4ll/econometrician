...
The measurement/reference roles and precision boundary are stated below. The
structural Idriç attempt and the complete finite geometric/error model are in
...
This is a binary32 endpoint-pair path, not yet the complete physical numeric
layer. The broader policy uses Float16 measurement storage where supported,
explicit receipted promotion to Float32 for finite rotations, trigonometry,
Jacobians, or accumulation, and a named arithmetic-error source. That full
finite geometric kernel and its arithmetic bound remain to be implemented.

This narrow executable contract is the final endpoint-pair link. The broader
procedure first inventories every error source, then composes steering command,
`SO(3)` wheel rotation, `S^2` wheel normal, gauge projection, and this scalar
link. It works with the nominal 17.4:1 command map plus explicit model
uncertainty when direct yaw cannot be measured.
